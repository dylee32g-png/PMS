...
rem 트레이 상주 (2026-09-01) - 전용 창을 작업표시줄에서 숨기고 시계 옆 아이콘으로만 관리
rem   (공용 PC에서 직원들이 창을 만지는 사고 방지 - 더블클릭하면 10분간 창 확인 가능)
...
if errorlevel 1 (echo   [!] 트레이 상주 등록 실패) else (echo   [6-1] 트레이 상주(창 숨김 관리) 등록 완료)
...
echo    ★ 이 창은 20분 뒤부터 화면에서 숨겨지고, 시계 옆 트레이 아이콘으로만 남습니다.
echo       확인하려면 : 시계 옆 크롬 모양 아이콘 더블클릭 (10분 뒤 자동 숨김)
echo       (작업표시줄에 창이 없으니 다른 직원이 만질 일이 없습니다)
echo.
echo.
pause
